...
4.10. [Interfaces gráficas APK](#410-interfaces-gráficas--capturas-simuladas-en-dispositivo-móvil)
6.7. [Protocolo CIM completo](#67-protocolo-cim--especificación-técnica-completa)
9.1. [Tests detallados (30)](#91-descripción-detallada-de-los-30-tests-automatizados)
14.1. [Bitácora completa](#141-bitácora-completa-del-proyecto-método-científico)
19–22. [Extensiones, laboratorio, carga, glosario](#19-extensiones-cursor--aplicación-detallada-al-proyecto)
...
*Figura 3.1 — Arquitectura hub-and-spoke: apps, core-network y ESP32.*

![Flujo distribuido aplicaciones CIM](assets/imagenes/cim_flujo_app.png)

*Figura 3.2 — Flujo de datos entre hub TCP, estaciones y nodos Bluetooth.*

![Simulación ESP32 Wokwi — DHT22, LCD I2C, HTTP](assets/imagenes/esp32_wokwi_simulacion.png)

*Figura 3.3 — Proyecto de simulación `simulacion_esp32/` (telemetría; protocolo CIM BT en firmware producción).*
...
Esta sección documenta la experiencia de usuario (UX) industrial de cada APK tal como se presenta en un smartphone Android 10+ (API 29 mínimo, recomendado 1080p y 4 GB RAM). Las figuras son representaciones de la interfaz Jetpack Compose Material 3 con tema oscuro industrial (azul acero `#2c5282`, fondos `#1a202c`).
...
*Figura 4.1 — Coordinador: servidor hub en puerto 8888, lista de nodos ONLINE/OFFLINE, IP local visible para vincular estaciones.*
...
*Figura 4.2 — PLC: control arranque/parada cinta, matriz origen→destino, FAB Bluetooth inferior derecho.*
...
*Figura 4.3 — Calidad: CameraX + OpenCV ArUco DICT_4X4_50, recuadros verdes, contadores aprobados/rechazados.*
...
*Figura 4.4 — Manufactura: jogging ejes X/Y, posiciones HOME/READY, potencia/velocidad láser, recepción G-Code.*
...
*Figura 4.5 — Almacén: grid 18 celdas (3 niveles × 6 columnas), estado vacío/ocupado/en movimiento.*
...
*Figura 4.6 — Diálogo escaneo: dispositivos filtrados ESP32/CIM/NODO, botón Conectar por MAC.*
...
**Opción 1 — Markdown Preview Enhanced (recomendado en Cursor):**

1. Abrir este archivo en Cursor/VS Code.
2. `Ctrl+Shift+P` → `Markdown Preview Enhanced: Open Preview`.
3. Clic derecho en preview → **Chrome (Puppeteer) / PDF** → seleccionar stylesheet `docs/styles/industrial_pdf.css`.
4. Guardar como `docs/ENTREGA_FINAL_LEONARDO_ARAYA.pdf`.

**Opción 2 — Marp CLI:**

```powershell
cd docs
npx @marp-team/marp-cli ENTREGA_FINAL_LEONARDO_ARAYA.md -o ENTREGA_FINAL_LEONARDO_ARAYA.pdf --html --allow-local-files
```

**Opción 3 — Pandoc (si instalado):**

```powershell
pandoc ENTREGA_FINAL_LEONARDO_ARAYA.md -o ENTREGA_FINAL_LEONARDO_ARAYA.pdf --css=styles/industrial_pdf.css -V geometry:margin=2.5cm
```
